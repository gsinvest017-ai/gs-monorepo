...
## M4 — trading-SySTEM 加入 submodule

- SSH 進 gb10（100.87.199.126，user: gsinvest000）確認路徑存在且已是 git repo
- remote 已指向 `github.com/gsinvest017-ai/trading-SySTEM.git`，local 超前 5 commits
- `git push origin dev` from gb10 成功推送
- `git submodule add -b dev https://github.com/gsinvest017-ai/trading-SySTEM.git` 成功
- submodule SHA: ccf8445（heads/dev）

## M3 — gs-scraper 加入 submodule

- 發現 `/home/kevin/gs-scraper/QUANTDATA` 本身即為 git repo，remote 已指向 `github.com/gsinvest017-ai/gs-scraper`
- 該 GitHub repo 非空（已有 push）
- `git submodule add https://github.com/gsinvest017-ai/gs-scraper.git gs-scraper` 成功
- submodule SHA: 0087b82（heads/main）
